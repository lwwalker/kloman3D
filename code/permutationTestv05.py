...
def main(argv):
	...
	csvFN = outputfilename.split(".")[0]+".csv"
	csvFile = open(csvFN, 'w')
	colNames = ["Run","selected","accuracy","reg"]
	csvFile.write(','.join(colNames)+"\n")

	for j in range(100):
		print "Starting permutation {} of {}".format(j+1, 100)
		...
		while goFlag:
			...
			if workingModel.acc <= lastAcc:
				goFlag = False
				workingModel = lastModel
				print "Cycle {} result not better than cycle {}, keeping previous result.".format(i, i-1)
				print workingModel.prn()
			else:
				lastModel = workingModel.copy()
				lastModel.acc = runSVM(lastModel)
				lastAcc = lastModel.acc
				print "Cycle {} improved from previous cycle, continue iterating.".format(i)
				print workingModel.prn()	
			i += 1

		print "\nFinal Model"
		print workingModel.prn()
		csvFile.write(','.join([str(j+1), '|'.join([str(x) for x in workingModel.mySlice]), str(workingModel.pr)])+"\n")
